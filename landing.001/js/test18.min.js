...
///////////////////////////////////////////////
    ...
    function BB_orderBillSender(){
        ...
        let bb_sessionOrder = localStorage.getItem('bb_orderData');
        bb_sessionOrder = (typeof bb_sessionOrder === 'string') ? JSON.parse(bb_sessionOrder) : bb_orderRecord;
console.log(bb_sessionOrder);
        bb_contactName.value = bb_sessionOrder.clientName;
        bb_contactPhone.value = bb_sessionOrder.clientPhone;
        bb_contactAddress.value = bb_sessionOrder.clientAddress;
        bb_contactMessageOriginal.textContent = bb_sessionOrder.clientMessage;
        ...
        [bb_contactName, bb_contactPhone, bb_contactAddress].forEach((item, index) => {
            item.addEventListener('input', (event) => {
                ...
                if(bb_variable){
                    if(index === 0) bb_orderRecord.clientName = item.value;
                    if(index === 1) bb_orderRecord.clientPhone = item.value;
                    if(index === 2) bb_orderRecord.clientAddress = item.value;
                    item.style.cssText = 'background-color: none';
                    bb_notification.textContent = '';
                }else{
                    if(index === 0) bb_orderRecord.clientName = '';
                    if(index === 1) bb_orderRecord.clientPhone = '';
                    if(index === 2) bb_orderRecord.clientAddress = '';
                ...
                if(bb_orderRecord.clientName && bb_orderRecord.clientPhone && bb_orderRecord.clientAddress){
                    const bb_breaker = '\n\n'+'-'.repeat(50)+'\n\n';
                    ...
                    // bb_facebookPixel.fbtrack(bb_sessionOrder); // Cung cấp dữ liệu cho Facebook pixel
                    console.log(111, bb_orderRecord.clientMessage);
                }else{
                    bb_orderRecord.clientMessage = '';
                    bb_contactMessageOriginal.textContent = '';
                    bb_formSubmission.value = 'Thiếu thông tin';
                    bb_formSubmission.style = 'background-color: #c36887; width: 220px;';
                    console.log(222, bb_orderRecord.clientMessage);
                }
            });
        });
        ...
        bb_formSubmission.addEventListener('click', (event) => {
            event.preventDefault();
            if(bb_contactMessageOriginal.textContent){
                console.log(333, bb_orderRecord.clientMessage);
                ...
                console.log(444, bb_orderRecord.clientMessage);
                bb_formSubmission.value = 'Thiếu thông tin';
                bb_formSubmission.style = 'background-color: #c36887; width: 220px;';
            }
        });
