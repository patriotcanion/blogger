...
function BB_modalOderAndContactForm(){
    ...
    let bb_sessionOrderRecord = localStorage.getItem('bb_orderData');
    console.log(11111111111, typeof bb_sessionOrderRecord)
    if(typeof bb_sessionOrderRecord === 'object'){
        bb_sessionOrderRecord = localStorage.setItem('bb_orderData', JSON.stringify({
    ...
    } else if(typeof bb_sessionOrderRecord === 'string')
        bb_sessionOrderRecord = JSON.parse(bb_sessionOrderRecord);
...
///////////////////////////////////////////////
    ...
    function BB_orderBillSender(){
        ...
        const bb_regexName = /[^\w\sáàãảạăắằẵẳặâấầẫẩậđéèẽẻẹêếềễểệíìĩỉịóòõỏọôốồỗổộơớờỡởợúùũủụưứừữửựýỳỹỷỵÁÀÃẢẠĂẮẰẴẲẶÂẤẦẪẨẬĐÉÈẼẺẸÊẾỀỄỂỆÍÌĨỈỊÓÒÕỎỌÔỐỒỖỔỘƠỚỜỠỞỢÚÙŨỦỤƯỨỪỮỬỰÝỲỸỶỴ]/g;
        const bb_regexAddress = /[^\w\s\+\-\(\)\\\/&\.,áàãảạăắằẵẳặâấầẫẩậđéèẽẻẹêếềễểệíìĩỉịóòõỏọôốồỗổộơớờỡởợúùũủụưứừữửựýỳỹỷỵÁÀÃẢẠĂẮẰẴẲẶÂẤẦẪẨẬĐÉÈẼẺẸÊẾỀỄỂỆÍÌĨỈỊÓÒÕỎỌÔỐỒỖỔỘƠỚỜỠỞỢÚÙŨỦỤƯỨỪỮỬỰÝỲỸỶỴ]/g;
        const bb_requiredMatcher = []; // Check name, phone, address
        let bb_actionFlag = 0;

        bb_contactName.value = bb_sessionOrderRecord.clientName;
        bb_contactPhone.value = bb_sessionOrderRecord.clientPhone;
        bb_contactAddress.value = bb_sessionOrderRecord.clientAddress;
        bb_contactMessageOriginal.textContent = bb_sessionOrderRecord.clientMessage;

        if(bb_contactName.value && bb_contactPhone.value && bb_contactAddress.value && bb_contactMessageOriginal.textContent) bb_actionFlag = 1;
        ...
        [bb_contactName, bb_contactPhone, bb_contactAddress].forEach((item, index) => {
            item.addEventListener('input', (event) => {
                ...
                if(index === 0){
                    item.value = item.value.replace(bb_regexName, '');
                    ...
                    item.value = item.value.replace(bb_regexAddress, '');
                    bb_variable = item.value.length >= 10;
                ...
                /** Khi các thông tin đẫ điền đủ thì gửi form đến email */
                bb_actionFlag = bb_requiredMatcher.reduce((product, value) => {return product * value});
                if(bb_actionFlag){
                    const bb_breaker = '\n\n'+'-'.repeat(50)+'\n\n';
                    ...
                    bb_sessionOrderRecord.clientName = bb_contactName.value;
                    bb_sessionOrderRecord.clientPhone = bb_contactPhone.value;
                    bb_sessionOrderRecord.clientAddress = bb_contactAddress.value;
                    bb_sessionOrderRecord.clientMessage =
                        ...
                        `Lưu file CSV:\n"${bb_today}","${bb_time}","${bb_sessionOrderRecord.fullName}","${bb_sessionOrderRecord.quantity}","${bb_sessionOrderRecord.sumTotal}","${bb_sessionOrderRecord.shipCost}","${bb_contactName.value}","${bb_contactPhone.value}","${bb_contactAddress.value}"${bb_breaker}` +
                        `${bb_contactMessage.value}`;
                    bb_contactMessageOriginal.textContent = bb_sessionOrderRecord.clientMessage;
                    localStorage.setItem('bb_orderData', JSON.stringify(bb_sessionOrderRecord));
                    bb_formSubmission.value = 'Mua hàng';
                    bb_formSubmission.style = 'background-color: #3786C9; width: 180px;';
                    bb_facebookPixel.fbtrack(bb_sessionOrderRecord); // Cung cấp dữ liệu cho Facebook pixel
                }else{
    ...
    function BB_priceEstimator(_seekedQuantity){
        ...
        bb_sessionOrderRecord.quantity = _seekedQuantity;
        bb_sessionOrderRecord.sumTotal = _lastTotalCost + _shipCost;
        bb_sessionOrderRecord.shipCost = _shipCost;
    }
