...
function BB_landingPageContactAdder(){
	...
	let bb_phoneNode = document.createElement('p');
	bb_phoneNode.textContent = '(Số điện thoại có sử dụng Zalo)';
	bb_sellerPhone.after(bb_phoneNode);
	bb_sellerEmail.innerHTML = bb_sellerEmail.innerHTML + ' ' + bb_sellerContactData.email;

	[bb_sellerPhone, bb_sellerEmail].forEach((item, index) => {
		item.addEventListener('click', function (event){
			event.preventDefault();
			if(index === 0) window.open('tel:+84' + bb_sellerContactData.phone);
			if(index === 1) window.open('mailto:' + bb_sellerContactData.email);
		});
	});
}
