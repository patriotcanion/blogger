...
function BB_priceEstimator(_seekedQuantity){
	...
	if(bb_productData.options.active){
		...
		_percent = _percent.filter(Boolean).reduce((prev, curr) => {return prev > curr ? prev : curr;});
...
// console.log('Số lượng: ', _seekedQuantity);
// console.log('Giá ĐV: ', bb_productData.product.price);
// console.log('Tổng giá chưa giảm: ', _calculated);
// console.log('% chiết khấu: ', bb_productData.promotion.percent);
// console.log('% chiết khấu: ', bb_productData.promotion.percent);
// console.log('% chiết khấu: ', bb_productData.promotion.percent);
// console.log('% chiết khấu: ', bb_productData.promotion.percent);
	bb_productPrice.innerHTML = `${BB_numberWithCommas(_lastTotalCost / _seekedQuantity, 1000)}<sup>đ</sup>`;
	...
	bb_billLastTotalCost.innerHTML = `${BB_numberWithCommas(_lastTotalCost + _shipCost, 1000)}<sup>đ</sup>`;
}
...
function BB_landingPageDisplayer(){
	...
	if(bb_combosArea && bb_productData.options.active){
		...
		[	bb_combosArea.querySelector('.regular'),
			bb_combosArea.querySelector('.popular-plan'),
			bb_combosArea.querySelector('.best-value-plan')
