...
function BB_priceEstimator(_seekedQuantity){
	...
	bb_productPrice.innerHTML = `${BB_numberWithCommas(_lastTotalCost / _seekedQuantity, 1000)}<sup>đ</sup> \
		<s>${BB_numberWithCommas(bb_productData.product.price, 1000)}<sup>đ</sup></s>`;
...
function BB_itemQuantitySelector(_firstQuantity){
	...
	bb_productQuantityArea.querySelector('button:nth-child(1)').addEventListener('click', (event) => {
		event.preventDefault();
		const _number = parseInt(bb_billQuantity.value, 10);
		if(_number > 1 && bb_quantityCounter > 1){
			bb_quantityCounter--;
			bb_billQuantity.value = (_number - bb_quantityCounter) === 1 ? bb_quantityCounter : _number - 1;
			BB_priceEstimator(bb_quantityCounter);
		}
	});
